...
class UI:
    ...
    def __init__(self):
        ...
        self.health_bar_rect = pygame.Rect(
            10, 10, UI_SETTINGS["HEALTH_BAR_WIDTH"], UI_SETTINGS["BAR_HEIGHT"]
        )
        self.energy_bar_rect = pygame.Rect(
            10, 38, UI_SETTINGS["ENERGY_BAR_WIDTH"], UI_SETTINGS["BAR_HEIGHT"]
        )

        self.dust_image = pygame.image.load(
            os.path.join(os.path.dirname(__file__), "..", "pics", "dust.png")
        ).convert_alpha()
        self.dust_image = pygame.transform.scale(self.dust_image, (30, 30))
        self.dust_rect = self.dust_image.get_rect(
            bottomright=(
                self.display_surface.get_size()[0] - 10,
                self.display_surface.get_size()[1] - 20,
            )
        )

    def show_bar(
        self, current: int, max_amount: int, bg_rect: "pygame.rect", color: ...
    ):
        """
        Draw the bar.
        ...
        pygame.draw.rect(
            self.display_surface, UI_SETTINGS["UI_COLORS"]["BG_COLOR"], back_rect, 0, 10
        )
        ...
        pygame.draw.rect(
            ...
            UI_SETTINGS["UI_COLORS"]["BORDER_COLOR"],
            back_rect,
            3,
            10,
        )
    ...
    def show_dust(self, dust: int):
        ...
        text_surf = self.font.render(
            str(int(dust)), False, UI_SETTINGS["UI_FONT_COLOR"]
        )
    ...
    def display(self, player: "Player"):
        ...
        self.show_bar(
            ...
            UI_SETTINGS["UI_COLORS"]["HEALTH"],
        )
        self.show_bar(
            player.energy,
            PLAYER_MAX_ENERGY,
            self.energy_bar_rect,
            UI_SETTINGS["UI_COLORS"]["ENERGY"],
        )
