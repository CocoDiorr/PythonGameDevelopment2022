...
translation = gettext.translation(
    "Companion",
    os.path.join(os.path.dirname(__file__), "..", "locale", "companion"),
...
class Companion(pygame.sprite.Sprite):
    ...
    def __init__(
        self, screen: "pygame.display", level: "Level", player: "Player" = None
    ):
        """
        ...
        self.rect = self.image.get_rect(
            midbottom=(
                WINDOW_RESOLUTION[0] - COMPANION_SIZE[0] // 2,
                WINDOW_RESOLUTION[1],
            )
        )
        ...
        # companion outline
        self.fill_box = pygame.Rect(
            self.rect.left - 5, self.rect.top - 5, self.rect.w + 5, self.rect.h + 5
        )
    ...
    def show_msg(self, screen: "pygame.display", msg: str):
        ...
        text_box = pygame.Rect(
            self.fill_box.left - box_width - 20,
            WINDOW_RESOLUTION[1] - box_height - 20,
            box_width + 30,
            box_height + 20,
        )
    ...
    def greeting(self, screen: "pygame.screen"):
        ...
        """
        text_surface_1 = self.font.render(
            _("Hi, haven't seen you in a while ! <3"), 0, COMPANION_COLORS["FONT_COLOR"]
        )
        text_surface_2 = self.font.render(
            _("Do you want a story ?"), 0, COMPANION_COLORS["FONT_COLOR"]
        )

        greet_rect = pygame.Rect(
            self.fill_box.left - text_surface_1.get_size()[0] - 20,
            self.fill_box.top,
        ...
        screen.blit(
            text_surface_2,
            (greet_rect.left + 15, greet_rect.top + 30 + text_surface_1.get_size()[1]),
        )

        yes = Button(
            self.level,
            (
                greet_rect.left + greet_rect.w * 0.2,
                greet_rect.top + greet_rect.h * 9 / 16,
            ),
            greet_rect.w * 0.25,
            greet_rect.h * 0.35,
            _("Yes"),
        ...
        no = Button(
            self.level,
            (
                greet_rect.left + greet_rect.w * 0.55,
                greet_rect.top + greet_rect.h * 9 / 16,
            ),
            greet_rect.w * 0.25,
            greet_rect.h * 0.35,
    ...
    def trade(self, screen: "pygame.display"):
        ...
        text_surface = self.font.render(
            _("How much would you pay ?"), 0, COMPANION_COLORS["FONT_COLOR"]
        )

        trade_rect = pygame.Rect(
            self.fill_box.left - text_surface.get_size()[0] * 2 + 4,
            self.fill_box.top,
        ...
        screen.blit(
            text_surface, (trade_rect.left + trade_rect.width // 4, trade_rect.top + 10)
        )
        buttons = []
        buttons.append(
            Button(
                self.level,
                (
                    trade_rect.left + trade_rect.w * 0.25,
        ...
        buttons.append(
            Button(
                self.level,
                (
                    trade_rect.left + trade_rect.w * 0.55,
                    trade_rect.top + trade_rect.h / 4 + 15,
                ),
                trade_rect.w * 0.25,
                trade_rect.h * 0.25,
    ...
    def display(self):
        ...
        pygame.draw.rect(
            self.screen, COMPANION_COLORS["MAIN_COLOR"], self.fill_box, 0, 20
        )
        pygame.draw.rect(
            self.screen, COMPANION_COLORS["OUTLINE_COLOR"], self.fill_box, 10, 20
        )
        gradient_rect = pygame.Rect.inflate(self.fill_box, -5, -5)
        pygame.draw.rect(
            self.screen, COMPANION_COLORS["GRADIENT"], gradient_rect, 3, 20
        )
