...
class Button:
    ...
    def __init__(
        ...
        self.font = pygame.font.Font(
            COMPANION_BUTTON["FONT"], COMPANION_BUTTON["FONT_SIZE"]
        )
        self.text = text
        self.text_surface = self.font.render(text, 0, COMPANION_BUTTON["FONT_COLOR"])
        self.rect = self.text_surface.get_rect(
            left=self.x - 12, top=self.y - 10, w=int(w), h=int(h)
        )
    ...
    def display(self, surface: "pygame.surface"):
        ...
        pygame.draw.rect(
            surface, COMPANION_BUTTON["OUTLINE_COLOR"], self.text_rect, 10, 10
        )
        surface.blit(
            self.text_surface,
            (
                self.rect.left + (self.rect.w - self.text_surface.get_size()[0]) // 2,
